...
def create_measure(request):
    if request.method == "POST":
        ...
        try:
            CorrectiveMeasure.objects.create(order=order, measure=measure)
...
def zone_agro_list(request):
    ...
    return render(
        request,
        "pages/zones_agro.html",
        {
            "zones": zones,
            "active_page": "zones_agro",
        },
    )
...
def index(request):
    ...
    successful_tests_user = Observation.objects.filter(
        success_according_user=True
    ).count()
    success_according_user_percentage = (
        (successful_tests_user / total_tests * 100) if total_tests > 0 else 0
    )
    failed_according_user_percentage = (
        (100 - success_according_user_percentage) if total_tests > 0 else 0
    )

    tests_by_zone = (
        Observation.objects.values("zone_agro__name")
    ...
    tests_by_day = (
        Observation.objects.filter(created_at__range=(start_date, end_date))
    ...
    users_by_zone = (
        Observation.objects.values("zone_agro__name")
        .annotate(user_count=Count("user_profile__id", distinct=True))
        .order_by("zone_agro__name")
    )
    ...
    tests_this_month = Observation.objects.filter(
        created_at__year=end_date.year, created_at__month=end_date.month
    ).count()
    ...
    context = {
        ...
        "success_according_user_percentage": round(
            success_according_user_percentage, 2
        ),
...
def get_users(request):
    users = (
        CustomUser.objects.annotate(test_count=Count("userprofile__observation"))
...
def get_user_images(request, user_id):
    observations = Observation.objects.filter(
        user_profile__user__id=user_id, image__isnull=False
    )
    images = [
        {"id": obs.id, "url": obs.image.url, "larval_stage": obs.larval_stage}
        ...
        for image_id in image_ids:
            try:
                observation = Observation.objects.get(
                    id=image_id, user_profile__user__id=user_id
                )
